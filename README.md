...
正式 Docker 镜像发布后，计划统一使用 `7017` 端口，由后端同时提供 API 和构建后的网页界面，因此部署时只需要暴露一个端口：
...
源码开发时使用后端 `7018`、前端 `7019` 两个端口；发布到 Docker 后只使用 `7017`。当前 Docker 配置仍在整理中，暂时不要把其它项目的镜像当作 Archiva Music 镜像使用。
